...
INSERT INTO auth_providers (internal_id, provider_key, name, status, created_at, updated_at, version)
...
INSERT INTO roles (internal_id, role_key, name, user_type)
...
INSERT INTO permissions (internal_id, permission_key, name, domain, created_at, updated_at, version)
VALUES
...
INSERT INTO role_permissions (internal_id, role_id, permission_id, created_at, updated_at, version)
...
(
    ...
    CONSTRAINT chk_staff_roles_type CHECK (
        (SELECT user_type
         FROM users
         WHERE id = user_id)
            IN ('MERCHANT_OWNER', 'BRANCH_MANAGER', 'STAFF')
        )
);
...
(
    ...
    device_type  character varying(10)  NOT NULL CHECK (device_type IN ('IOS', 'ANDROID', 'WEB')),
